...
output_dir = results_dir / "12_4_GRNN"
...
def grnn_predict(X_train, y_train, X, spread):
    d2 = np.sum((X[:, None, :] - X_train[None, :, :]) ** 2, axis=2)
    weights = np.exp(-d2 / (2 * spread ** 2))
...
for spread in np.linspace(0.08, 0.30, 111):
    pred = grnn_predict(X_train, y_train, X_val, spread)
    metrics = evaluate(y_val, pred)
    records.append({"spread": spread, **metrics})
...
result.to_excel(output_dir / "12_4_GRNN_metrics.xlsx", index=False)
tuning.sort_values("RMSE").to_excel(output_dir / "12_4_GRNN_tuning.xlsx", index=False)

prediction_df = val_df.copy()
prediction_df["Pred_GRNN"] = pred
prediction_df.to_excel(output_dir / "12_4_GRNN_predictions.xlsx", index=False)

joblib.dump(
    {"X_train": X_train, "y_train": y_train, "spread": spread},
    output_dir / "12_4_GRNN.joblib",
)

plot_result(y_val, pred, output_dir / "12_4_GRNN.png", "#8E44AD", "d")
