...
output_dir = results_dir / "12_3_GPR"
...
for length_scale in [0.3, 0.4, 0.5, 0.6, 0.8]:
    for noise_level in [0.001, 0.003, 0.005, 0.01, 0.02]:
        kernel = (
...
result = pd.DataFrame([{
    "Model": "GPR",
    "length_scale": tuning.loc[best_index, "length_scale"],
    "noise_level": tuning.loc[best_index, "noise_level"],
    **metrics,
    "Paper_R2": PAPER_R2,
...
result.to_excel(output_dir / "12_3_GPR_metrics.xlsx", index=False)
tuning.sort_values("RMSE").to_excel(output_dir / "12_3_GPR_tuning.xlsx", index=False)

prediction_df = val_df.copy()
prediction_df["Pred_GPR"] = pred
prediction_df.to_excel(output_dir / "12_3_GPR_predictions.xlsx", index=False)

joblib.dump(best_model, output_dir / "12_3_GPR.joblib")

plot_result(y_val, pred, output_dir / "12_3_GPR.png", "#8E44AD", "c")
