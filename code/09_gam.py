...
output_dir = results_dir / "09_4_GAM"
...
def make_gam(n_knots, degree, alpha):
...
for n_knots in [20, 40, 80, 120, 160, 180]:
    for degree in [2, 3]:
        for alpha in [0.01, 0.03, 0.05, 0.1, 0.2, 0.3, 0.5, 1.0]:
...
result.to_excel(
    output_dir / "09_4_GAM_metrics.xlsx",
    index=False,
)

pd.DataFrame(records).sort_values("RMSE").to_excel(
    output_dir / "09_4_GAM_tuning.xlsx",
    index=False,
)

prediction_df = val_df.copy()
prediction_df["Pred_GAM"] = best_pred
prediction_df.to_excel(
    output_dir / "09_4_GAM_predictions.xlsx",
    index=False,
)

joblib.dump(
    best_model,
    output_dir / "09_4_GAM.joblib",
)
